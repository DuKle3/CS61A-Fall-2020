...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        ...
        rest = eval_all(expressions.rest, env)
        if rest is None:
            return first 
        return rest 
    # END PROBLEM 7
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.bindings[function_name] = do_lambda_form(Pair(formals, body), env)
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            eval_all_value = eval_all(expressions.first.rest, env)

            if expressions.first.rest is not nil:
                return eval_all_value
            else:
                return test
