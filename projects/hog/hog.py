...
def swine_align(player_score, opponent_score):
    ...
    if player_score and opponent_score:
        divisor: int = min(player_score, opponent_score)
        while divisor > 0:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def compare_the_hightest(score0, score1):
        def print_score(score):
            cmp = score - last_score
            if cmp > running_high:
                print(cmp, 'point(s)! The most yet for Player', who)
                return announce_highest(who, score, cmp)
            else:
                return announce_highest(who, score, running_high)
        if who:
            return print_score(score1)
        else:
            return print_score(score0)
    return compare_the_hightest
    # END PROBLEM 7
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    condition1 = extra_turn_strategy(score, opponent_score, cutoff=6, num_rolls=4)
    if score > opponent_score:
        return extra_turn_strategy(score, opponent_score, cutoff=6, num_rolls=3)
    return condition1  # Replace this statement
    # END PROBLEM 12
