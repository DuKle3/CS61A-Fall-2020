...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        leaf_place, transitions = self.place, 0
        assert leaf_place ,"leaf_place is None"
        while leaf_place != beehive:
            if self.min_range <= transitions <= self.max_range:
                if leaf_place.bees == []:
                    leaf_place = leaf_place.entrance
                    transitions += 1
                else:
                    return rANTdom_else_none(leaf_place.bees)
            else:
                leaf_place = leaf_place.entrance
                transitions += 1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for each_bee in self.place.bees[:]:
    ...
    def action(self, gamestate):
        ...
        if self.true_queen == False:
            ...
            while currentplace != None:
                if currentplace.ant != None and currentplace.ant.buffered != True:
                    currentplace.ant.damage *= 2 
                    currentplace.ant.buffered = True
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant is None:
            return None
        return self.place.ant.blocks_path        # END Problem Optional
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if self.direction is False and not isinstance(self.place.entrance, Hive):
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        ...
        if self.contained_ant is None:
            if isinstance(other, ContainerAnt) == False and other.can_contain(self) == False:
                return True
        return False 
        # END Problem Optional 2
...
def apply_status(status, bee, length):
    ...
    new_action = status(prev_action, bee) 
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while current_place is not beehive:
            if current_place.ant is not None and current_place.ant != self:
                insects_in_front_dict[current_place.ant] = distance
            elif len(current_place.bees) != 0:
                for each_bee in current_place.bees:
                    insects_in_front_dict[each_bee] = distance 
