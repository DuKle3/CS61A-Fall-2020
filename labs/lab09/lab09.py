...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        # 1 + [2, 3] subseqs
        sub_seq = subseqs(s[1:])
        return sub_seq + insert_into_all(s[0], sub_seq)
...
class Keyboard:
    ...
    def typing(self, typing_input):
        ...
        for pos in typing_input:
            typed += str(self.buttons[pos].key)
...
def shuffle(cards):
    ...
    for i in range(len(cards)):
        shuffled.append(cards[(i % 2) * half + i // 2])
    return shuffled
...
def insert(link, value, index):
    ...
    """
    if index == 0:
        # mutate current_link and create a new Link behind it.
