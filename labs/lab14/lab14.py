...
def prune_min(t):
    ...
    if t.is_leaf():
        ...
        t.branches = [min([b for b in t.branches], key=lambda x: x.label)]
...
def num_splits(s, d):
    ...
    "Build Set1 / Set2 From Scratch, Track the difference."
    def num_splits_helper(s, difference):
        if len(s) == 0:
            return 1 if abs(difference) <= d else 0 
        subset_1_have_first = num_splits_helper(s[1:], difference + s[0])
        subset_2_have_first = num_splits_helper(s[1:], difference - s[0])
        return subset_1_have_first + subset_2_have_first

    return num_splits_helper(s, 0) // 2 # helper will count twice
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        insert_match, insert_cost = '+['+code_char+']' + h[0], h[1] + 1
        possibilities.append((insert_match, insert_cost))

        # Delete
        h = helper_align(skeleton_idx + 1, code_idx) 
        delete_match, delete_cost = '-['+skel_char+']' + h[0], h[1] + 1
        possibilities.append((delete_match, delete_cost))
